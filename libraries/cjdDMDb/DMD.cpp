		     ...
		     byte bGraphicsMode)
{
	...
	if (cX >= (DMD_PIXELS_ACROSS*DisplaysWide) || cY >= DMD_PIXELS_DOWN)
	    return;
    }
}
...
void DMD::drawMarquee(const char *bChars, byte length, byte top)
{
    ...
    marqueeOffset = 64;//(DMD_PIXELS_ACROSS * 2);	//DisplaysWide
...
boolean DMD::stepMarquee(int amount)
{
    ...
    if (marqueeOffset < -(marqueeWidth)) {
	    marqueeOffset = 64;//(DMD_PIXELS_ACROSS * 2);
	    clearScreen(true);
        ret=true;
    }
...
int DMD::drawChar(int bX, int bY, const char letter, byte bGraphicsMode)
{
    ...
    if (bX >= (DMD_PIXELS_ACROSS*DisplaysWide) || bY > (DMD_PIXELS_DOWN*DisplaysHigh) || bX < -width
	|| bY < -height)
	return -1;
